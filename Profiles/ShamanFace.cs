...
namespace SmartBotProfiles
{
    ...
    public class ShamanFace : Profile
    {
        ...
        {
            using (
              StreamWriter finleyChoiceStreamWriter =
                new StreamWriter(AppDomain.CurrentDomain.BaseDirectory + "\\Profiles\\FinleyChoices.txt", true))
            {
                List<KeyValuePair<Card.Cards, int>> filteredTable =
                  _heroPowersPriorityTable.Where(x => choices.Contains(x.Key)).ToList();
                foreach (var q in filteredTable)
                {
                    finleyChoiceStreamWriter.WriteLine("{0} ------ {1} // {2}", q.Key, q.Value, CardTemplate.LoadFromId(q.Key).Name);
                }
                finleyChoiceStreamWriter.WriteLine("[Best Choice according to SmartBot] {0}",
                  CardTemplate.LoadFromId(filteredTable.First(x => x.Value == filteredTable.Max(y => y.Value)).Key).Name);
                finleyChoiceStreamWriter.WriteLine("==============================");
                finleyChoiceStreamWriter.WriteLine(Bot.CurrentBoard.ToSeed());
                finleyChoiceStreamWriter.WriteLine("\n==============================");
